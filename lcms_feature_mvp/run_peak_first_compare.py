...
function localPeakAlignmentEnabled(){ return !!($("showAligned")?.checked && $("showLocalPeakAligned")?.checked && selectedPeak()); }
function hexToRgba(hex, alpha){
  const m=String(hex||"").replace("#",""); if(m.length!==6)return `rgba(37,99,235,${alpha})`;
...
function drawSelectedPeakBands(ctx,p,pk,series){
  if(!pk)return;
  const laneH=Math.max(12,(p.bottom-p.top)/Math.max(series.length,1));
  const localMode=localPeakAlignmentEnabled();
  series.forEach((s,i)=>{
    const band=samplePeakBand(pk,s.sample);
    const x0=p.toX(band.start), x1=p.toX(band.end);
    if(x1<p.left||x0>p.right)return;
    const left=Math.max(p.left,Math.min(x0,x1)), right=Math.min(p.right,Math.max(x0,x1));
    const y0=p.top+i*laneH, y1=Math.min(p.bottom,y0+laneH);
    ctx.save();
    ctx.fillStyle=hexToRgba(s.color,localMode?0.20:0.16);
    ctx.fillRect(left,y0,Math.max(1,right-left),Math.max(8,y1-y0));
    ctx.strokeStyle=hexToRgba(s.color,0.70);
    ctx.setLineDash([4,3]);
    ctx.strokeRect(left,y0,Math.max(1,right-left),Math.max(8,y1-y0));
    ctx.restore();
  });
  const x0=p.toX(Number(pk.rt_start)), x1=p.toX(Number(pk.rt_end));
  if(x1>=p.left&&x0<=p.right){
    ctx.save();
    ctx.strokeStyle="#111827";
    ctx.setLineDash([6,4]);
    ctx.strokeRect(Math.max(p.left,Math.min(x0,x1)),p.top,Math.min(p.right,Math.max(x0,x1))-Math.max(p.left,Math.min(x0,x1)),p.bottom-p.top);
    ctx.restore();
  }
}
...
function drawChrom(){
  ...
  $("showLocalPeakAligned").disabled=!$("showAligned").checked||!selected;
  drawSelectedPeakBands(ctx,p,selected,series);
  ...
  const localText=localPeakAlignmentEnabled()?"local peak RT correction ON: curves are shifted by the selected peak local RT shift; colored bands are sample-specific integration windows.":"colored bands show each sample's actual integration window for the selected TIC peak.";
  canvas._plot=p; $("chromInfo").textContent=`${DATA.peak_results.length} compared TIC peaks; ${localText} Small ticks mark other compared peak apices.`;
