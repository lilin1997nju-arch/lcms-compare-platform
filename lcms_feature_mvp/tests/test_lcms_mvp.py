...
class LCMSMvpTests(unittest.TestCase):
    def test_peak_first_template_marks_selected_feature_rt_and_uses_one_tic_band(self) -> None:
        ...
        self.assertIn("颜色按截断后的 |ln(比较/参比)| 显示", PEAK_FIRST_TEMPLATE)
        ...
        self.assertIn("方块面积表示两样本中较高的归一化丰度", PEAK_FIRST_TEMPLATE)
        ...
        self.assertIn("修饰水平与蛋白形式差异定量", PEAK_FIRST_TEMPLATE)
        self.assertIn("一级序列覆盖与三级结构定位", PEAK_FIRST_TEMPLATE)
        self.assertIn('id="moduleNavList"', PEAK_FIRST_TEMPLATE)
        self.assertIn('id="analysisContextItems"', PEAK_FIRST_TEMPLATE)
        self.assertIn('id="peakInspectionModule"', PEAK_FIRST_TEMPLATE)
        self.assertIn("function moveReportModule", PEAK_FIRST_TEMPLATE)
        self.assertIn("function toggleModuleCollapse", PEAK_FIRST_TEMPLATE)
        self.assertIn("button.onmousedown", PEAK_FIRST_TEMPLATE)
        self.assertIn("module-nav-drag-ghost", PEAK_FIRST_TEMPLATE)
        self.assertIn("module-drop-target", PEAK_FIRST_TEMPLATE)
        self.assertIn("button.blur()", PEAK_FIRST_TEMPLATE)
        self.assertNotIn(".module-nav-rail:focus-within", PEAK_FIRST_TEMPLATE)
        self.assertIn("function modFamilyVisual", PEAK_FIRST_TEMPLATE)
        self.assertIn("mod-proteolytic-family", PEAK_FIRST_TEMPLATE)
        self.assertIn("family-last-member", PEAK_FIRST_TEMPLATE)
        self.assertIn("mod-proteolytic-member-detail", PEAK_FIRST_TEMPLATE)
        self.assertIn("mod-quant-standalone", PEAK_FIRST_TEMPLATE)
        self.assertIn("function persistReportSettings", PEAK_FIRST_TEMPLATE)
        self.assertIn("window.scrollBy(0,offset)", PEAK_FIRST_TEMPLATE)
        self.assertIn("function renderAnalysisContext", PEAK_FIRST_TEMPLATE)
