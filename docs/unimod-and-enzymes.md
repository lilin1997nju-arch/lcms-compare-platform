...
在 MS/MS 的“证据来源”中选择 Unimod 探索候选后，不易理解的修饰名称
带虚线下划线；鼠标停留在名称或已选候选框上，可查看中文解释、
Unimod 原始完整名称和编号。涵盖经核对的特殊缩写、试剂衍生物、
糖组成和金属加合物；氧化、脱酰胺、常规固定烷基化等不添加说明。
尚未整理的名称不会根据字面猜测其化学含义。

说明由 `core/lcms_unimod_annotations.py` 根据离线数据库编号生成，
只用于展示，不参与候选生成、评分或定量，也不代表该修饰或位点已被确认。
新版平台打开已有任务结果即可使用，无需重新计算；已导出的旧 HTML
不会自动更新，需通过新版模板重新生成报告。桌面安装版需同步更新代码或安装包。
